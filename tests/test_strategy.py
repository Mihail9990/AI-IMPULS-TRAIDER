...
from trader.events import (
    ...
    normalize_event, normalize_events, protection_range_diagnostic,
)
from trader.execution import ExecutionPolicy, is_crossed_level_rejection, trigger_level_passed
from trader.model import (
    CycleState, Leg, remaining_recovery_distance, stop_slippage, target_for,
    trigger_slippage,
)
...
class ScenarioParameterStrategyTest(unittest.TestCase):
    ...
    def test_first_increase_slippage_is_divided_after_old_slippage_but_not_trigger_slippage(self):
        ...
        self.assertEqual(state.general_recovery, before + D("41"))
...
class EntryRetryTest(unittest.TestCase):
    ...
    def test_trigger_and_market_fallback_use_next_scenario_parameters(self):
        ...
        self.assertEqual(args[3:], (D("4010.50"), D("4007.50"), D("4013.50")))
    ...
    def test_fast_continuation_stop_uses_current_scenario_and_carried_recovery(self):
        ...
        self.assertEqual(bot.state.general_recovery, D("0.215"))
    ...
    def test_continuation_recovery_uses_losses_target_and_new_spread_once(self):
        ...
        self.assertEqual(bot.state.long.take_profit, D("4376.20"))
        self.assertEqual(bot.state.short.take_profit, D("4374.00"))
    ...
    def test_projected_trigger_target_uses_trigger_entry_not_opposite_stop(self):
        ...
        bot.capital.working_stop.assert_called_once_with(
            "GOLD", "SELL", D("0.1"), D("4010.00"), D("4011.00"), D("4008.30")
        )
    ...
    def test_fast_positions_path_uses_broker_order_when_sl_precedes_reentry(self):
        ...
        bot.capital.activity.return_value = [
            {"dateUTC": "2026-09-21T10:00:02Z", "dealId": "short-new",
             "source": "USER", "status": "ACCEPTED", "type": "POSITION",
             "details": {"workingOrderId": "sell-trigger", "direction": "SELL",
                         "level": "4010.00"}},
    ...
    def test_equal_or_missing_broker_times_do_not_guess_scenario(self):
        for close_time in (None, "2026-09-21T10:00:02Z"):
            ...
            opening = {"dealId": "short-new", "source": "USER", "status": "ACCEPTED",
                       "type": "POSITION", "dateUTC": "2026-09-21T10:00:02Z",
                       "details": {"workingOrderId": "sell-trigger", "direction": "SELL",
                                   "level": "4010.00"}}
            bot.capital.activity.return_value = [opening, event]
            self.assertTrue(bot._recover_trigger_fill_then_stop(
                {"short-new": position}, bot.state.long, stopped
            ))
    ...
    def test_trigger_404_stays_unresolved_until_delayed_execution_appears(self):
        ...
        bot.capital.positions.return_value = [{
            "position": {
                ...
                "direction": "SELL", "level": 4010.0, "size": 10,
            },
            "market": {"epic": bot.cfg.epic},
        }]
...
class GeneralRecoveryModelTest(unittest.TestCase):
    ...
    def test_s1_sl_trigger_sl_trigger_sequence(self):
        ...
        self.assertEqual(state.general_recovery, D("36"))
    ...
    def test_projected_reopen_is_pure_and_excludes_unknown_future_slippage(self):
        ...
        self.assertEqual((size, distance, projected), (D("10"), D("1.5"), D("0.4")))
    ...
    def test_scenario_eight_to_nine_accounts_pending_and_trigger_only(self):
        ...
        self.assertEqual(state.general_recovery, D("182"))
    ...
    def test_zero_general_recovery_valid_but_zero_size_rejected(self):
        state = CycleState(general_recovery=D("0"), recovery_model_version=3)
...
class GeneralRecoveryIntegrationFixTest(unittest.TestCase):
    ...
    def test_reporting_uses_general_not_legacy_leg_recovery(self):
        leg = Leg("SELL", D("4000"), D("4000"), size=D("20"), stop_distance=D("4"),
                  recovery=D("0"))
        text = leg_details(leg, general_recovery=D("102"), scenario=5)
        self.assertIn("recovery_distance=1.1", text)
...
class GeneralRecoveryV3AcceptanceTest(unittest.TestCase):
    SIZES = tuple(map(D, ("10", "10", "10", "10", "20", "20", "20", "20", "20")))
    DISTANCES = tuple(map(D, ("1", "1.5", "2", "3", "4", "4", "4", "4", "4")))
    DIRECTIONS = ("SELL", "SELL", "SELL", "BUY", "BUY", "SELL", "BUY", "SELL")
    SEQUENCE = tuple(map(D, ("8", "9", "20", "36", "37", "58", "59", "90", "92",
                              "174", "176", "217", "219", "301", "303", "385", "387")))
    TP_LEVELS = tuple(map(D, ("4002.40", "4004.10", "4006.30", "3990.90",
                               "3982.50", "4011.45", "3984.85", "4019.85")))
    RESULTS = tuple(map(D, ("8", "9", "10", "11", "13", "15", "17", "19")))

    def cfg(self):
        return Settings(
            size=D("10"), stop_distance=D("1"), target_profit=D("0.30"),
            scenario_sizes=self.SIZES, scenario_stop_distances=self.DISTANCES,
        )

    def begin(self):
        state = CycleState(cycle_id=301, cycle_attempt=1)
        strategy = Strategy(self.cfg(), state)
        strategy.begin(D("4000.50"), D("4000.00"))
        strategy.confirm_initial_fills(D("4000.50"), D("4000.00"))
        return strategy, state
    ...
    @staticmethod
    def sl_fill(leg):
        return leg.stop + (D("-0.10") if leg.direction == "BUY" else D("0.10"))

    @staticmethod
    def reopen_fill(leg):
        return leg.original_trigger_level + (
            D("0.10") if leg.direction == "BUY" else D("-0.10")
        )

    def test_model_version_three_and_initial_actual_recovery(self):
        strategy, state = self.begin()
        self.assertEqual(strategy.MODEL_VERSION, 3)
        self.assertEqual(state.recovery_model_version, 3)
        self.assertEqual((state.entry_spread, state.target_value, state.general_recovery),
                         (D("0.50"), D("3.00"), D("8.00")))

    def test_full_decimal_sequence_sizes_distances_and_immutable_anchors(self):
        strategy, state = self.begin()
        actual = [state.general_recovery]
        anchors = (state.long.original_trigger_level, state.short.original_trigger_level)
        for next_scenario, direction in enumerate(self.DIRECTIONS, 2):
            leg = state.long if direction == "BUY" else state.short
            survivor = state.short if direction == "BUY" else state.long
            survivor_size, survivor_entry = survivor.size, survivor.current_entry
            strategy.stopped(direction, self.sl_fill(leg), f"v3-sl-{state.scenario}-{direction}")
            actual.append(state.general_recovery)
            closure = state.pending_recovery[-1]
            self.assertEqual(closure["scenario_at_close"], next_scenario - 1)
            self.assertEqual(closure["d_accounted"], next_scenario > 2)
            self.assertFalse(closure["reentry_accounted"])
            strategy.reopened(
                direction, self.reopen_fill(leg), f"v3-deal-{next_scenario}-{direction}",
                f"v3-open-{next_scenario}-{direction}",
            )
            actual.append(state.general_recovery)
            self.assertEqual((state.long.original_trigger_level,
                              state.short.original_trigger_level), anchors)
            self.assertEqual((survivor.size, survivor.current_entry),
                             (survivor_size, survivor_entry))
            self.assertEqual(state.scenario, next_scenario)
        self.assertEqual(tuple(actual), self.SEQUENCE)
        self.assertEqual(state.short.size, D("20"))
        self.assertEqual((state.scenario, state.phase), (9, "SCENARIO_9_CLOSING"))

    def test_independent_tp_branches_and_cycle_results(self):
        import copy
        strategy, state = self.begin()
        for index, direction in enumerate(self.DIRECTIONS):
            leg = state.long if direction == "BUY" else state.short
            strategy.stopped(direction, self.sl_fill(leg), f"branch-sl-{state.scenario}-{direction}")
            survivor = state.short if direction == "BUY" else state.long
            self.assertEqual(survivor.take_profit, self.TP_LEVELS[index])
            branch = copy.deepcopy(state)
            Strategy(self.cfg(), branch).complete(survivor.direction, survivor.take_profit)
            self.assertEqual(branch.net_cycle_money, self.RESULTS[index])
            if index < len(self.DIRECTIONS) - 1:
                strategy.reopened(
                    direction, self.reopen_fill(leg), f"branch-deal-{index}",
                    f"branch-open-{index}",
                )

    def test_mirror_s5_and_s6_branches_keep_thirteen_and_fifteen(self):
        import copy
        for prefix, stopped_direction, expected in (
            (("BUY", "BUY", "BUY", "SELL"), "SELL", D("13")),
            (("BUY", "BUY", "BUY", "SELL", "SELL"), "BUY", D("15")),
        ):
            strategy, state = self.begin()
            for index, direction in enumerate(prefix, 2):
                leg = state.long if direction == "BUY" else state.short
                strategy.stopped(direction, self.sl_fill(leg), f"mirror-sl-{index}")
                strategy.reopened(
                    direction, self.reopen_fill(leg), f"mirror-deal-{index}",
                    f"mirror-open-{index}",
                )
            leg = state.long if stopped_direction == "BUY" else state.short
            strategy.stopped(stopped_direction, self.sl_fill(leg), "mirror-final-sl")
            survivor = state.short if stopped_direction == "BUY" else state.long
            branch = copy.deepcopy(state)
            Strategy(self.cfg(), branch).complete(survivor.direction, survivor.take_profit)
            self.assertEqual(branch.net_cycle_money, expected)
            self.assertEqual(
                abs(survivor.take_profit - survivor.current_entry) * survivor.size,
                max(state.general_recovery, survivor.stop_distance * survivor.size),
            )

    def test_s1_sl_pending_then_reentry_and_tp_before_trigger(self):
        import copy
        strategy, state = self.begin()
        strategy.stopped("SELL", D("4001.10"), "s1-sell-sl")
        closure = state.pending_recovery[-1]
        self.assertEqual(state.general_recovery, D("9"))
        self.assertEqual((D(closure["d_value"]), closure["d_accounted"],
                          closure["reentry_accounted"]), (D("10"), False, False))
        tp_branch = copy.deepcopy(state)
        Strategy(self.cfg(), tp_branch).complete("BUY", tp_branch.long.take_profit)
        self.assertEqual(tp_branch.general_recovery, D("9"))
        self.assertFalse(tp_branch.pending_recovery[-1]["d_accounted"])
        strategy.reopened("SELL", D("3999.90"), "s2-sell", "s1-linked-open")
        self.assertEqual(state.general_recovery, D("20"))
        self.assertTrue(closure["d_accounted"] and closure["reentry_accounted"])

    def test_s2_sl_accounts_distance_now_and_reentry_only_slippage(self):
        strategy, state = self.begin()
        strategy.stopped("SELL", D("4001.10"), "s1-sl")
        strategy.reopened("SELL", D("3999.90"), "s2-sell", "s2-open")
        strategy.stopped("SELL", self.sl_fill(state.short), "s2-sl")
        closure = state.pending_recovery[-1]
        self.assertEqual(state.general_recovery, D("36"))
        self.assertEqual((D(closure["d_value"]), closure["d_accounted"],
                          closure["reentry_accounted"]), (D("15"), True, False))
        events = len(state.recovery_events)
        strategy.reopened("SELL", D("3999.90"), "s3-sell", "s3-open")
        self.assertEqual(state.general_recovery, D("37"))
        self.assertTrue(closure["reentry_accounted"])
        self.assertEqual(len(state.recovery_events), events + 1)

    def test_general_greater_equal_and_less_than_base(self):
        strategy, state = self.begin()
        state.scenario = 5
        state.general_recovery = D("60")
        state.long.size, state.short.size = D("10"), D("20")
        state.long.stop_distance = state.short.stop_distance = D("4")
        before = (state.general_recovery, list(state.recovery_events), state.scenario)
        strategy.refresh_targets()
        self.assertEqual(state.long.take_profit - state.long.current_entry, D("6"))
        self.assertEqual(state.short.current_entry - state.short.take_profit, D("4"))
        self.assertEqual((D("6") * D("10"), D("4") * D("20")), (D("60"), D("80")))
        self.assertEqual((state.general_recovery, state.recovery_events, state.scenario), before)
        state.general_recovery = D("40")
        strategy.refresh_targets()
        self.assertEqual(state.long.take_profit - state.long.current_entry, D("4"))

    def test_projection_is_linked_pure_and_does_not_readd_s2_distance(self):
        strategy, state = self.begin()
        strategy.stopped("SELL", D("4001.10"), "project-s1")
        closure = state.pending_recovery[-1]
        before = (state.general_recovery, list(state.recovery_events), dict(closure), state.scenario)
        size, distance, recovery = strategy.projected_reopen("SELL")
        self.assertEqual((size, distance, recovery), (D("10"), D("1.5"), D("0.4")))
        self.assertEqual((state.general_recovery, state.recovery_events, closure, state.scenario), before)
        strategy.reopened("SELL", D("3999.90"), "project-s2", "project-open")
        strategy.stopped("SELL", D("4001.60"), "project-s2-sl")
        closure = state.pending_recovery[-1]
        before_general = state.general_recovery
        strategy.projected_reopen("SELL")
        self.assertTrue(closure["d_accounted"])
        self.assertEqual(state.general_recovery, before_general)

    def test_duplicate_sl_and_normal_reentries_are_idempotent(self):
        strategy, state = self.begin()
        strategy.stopped("SELL", D("4001.10"), "dup-s1-sl")
        strategy.reopened("SELL", D("3999.90"), "dup-s2", "dup-s1-open")
        strategy.stopped("SELL", D("4001.60"), "dup-s2-sl")
        snapshot = dict(state.pending_recovery[-1])
        before = (state.general_recovery, len(state.recovery_events), state.scenario)
        strategy.stopped("SELL", D("4001.60"), "dup-s2-sl")
        self.assertEqual((state.general_recovery, len(state.recovery_events), state.scenario), before)
        self.assertEqual(state.pending_recovery[-1], snapshot)
        strategy.reopened("SELL", D("3999.90"), "dup-s3", "dup-s2-open")
        after = (state.general_recovery, len(state.recovery_events), state.scenario)
        strategy.reopened("SELL", D("3999.90"), "dup-s3", "dup-s2-open")
        self.assertEqual((state.general_recovery, len(state.recovery_events), state.scenario), after)

    def test_restart_s1_and_s2_pending_reentry_contracts(self):
        strategy, state = self.begin()
        strategy.stopped("SELL", D("4001.10"), "restart-s1")
        with tempfile.NamedTemporaryFile() as file:
            state.save(file.name); restored = CycleState.load(file.name)
        Strategy(self.cfg(), restored).reopened(
            "SELL", D("3999.90"), "restart-s2", "restart-s1-open"
        )
        self.assertEqual(restored.general_recovery, D("20"))
        restored_strategy = Strategy(self.cfg(), restored)
        restored_strategy.stopped("SELL", self.sl_fill(restored.short), "restart-s2-sl")
        self.assertEqual(restored.general_recovery, D("36"))
        with tempfile.NamedTemporaryFile() as file:
            restored.save(file.name); again = CycleState.load(file.name)
        Strategy(self.cfg(), again).reopened(
            "SELL", D("3999.90"), "restart-s3", "restart-s2-open"
        )
        self.assertEqual(again.general_recovery, D("37"))

    def test_mandatory_s2_old_effective_stop_race_is_twenty_to_thirty_one(self):
        strategy, state = self.begin()
        state.scenario = 2
        state.general_recovery = D("20")
        leg = state.long
        leg.confirmed_stop = D("3999.50")
        leg.protection_readback = "ПОДТВЕРЖДЕНО"
        leg.stop_distance = D("1.5")
        leg.stop = D("3999.00")
        strategy.stopped(
            "BUY", D("3999.40"), "old-stop-after-reentry", scenario_at_close=2,
            broker_execution_time="2026-09-21T00:00:02+00:00",
        )
        closure = state.pending_recovery[-1]
        self.assertEqual(state.general_recovery, D("31"))
        self.assertEqual((closure["stop_distance"], closure["d_value"],
                          closure["d_accounted"]), ("1.00", "10.00", True))

    def test_late_s1_closure_projection_restart_reentry_and_duplicate(self):
        strategy, state = self.begin()
        strategy.stopped("SELL", D("4001.10"), "late-sell-s1")
        strategy.reopened("SELL", D("3999.90"), "late-sell-s2", "late-sell-open")
        self.assertEqual(state.general_recovery, D("20"))
        buy = state.long
        buy.confirmed_stop = D("3999.50")
        buy.protection_readback = "ПОДТВЕРЖДЕНО"
        strategy.stopped(
            "BUY", D("3999.40"), "late-buy-s1", scenario_at_close=1,
            broker_execution_time="2026-09-21T00:00:01+00:00",
        )
        closure = state.pending_recovery[-1]
        self.assertEqual(state.general_recovery, D("21"))
        self.assertFalse(closure["d_accounted"] or closure["reentry_accounted"])
        projected_general = state.general_recovery + D(closure["d_value"])
        self.assertEqual(projected_general, D("31"))
        before = (state.general_recovery, dict(closure), len(state.recovery_events), state.scenario)
        strategy.projected_reopen("BUY")
        self.assertEqual((state.general_recovery, closure, len(state.recovery_events), state.scenario), before)
        with tempfile.NamedTemporaryFile() as file:
            state.save(file.name); restored = CycleState.load(file.name)
        restored_strategy = Strategy(self.cfg(), restored)
        restored_strategy.reopened("BUY", D("4000.60"), "late-buy-s3", "late-buy-open")
        self.assertEqual(restored.general_recovery, D("32"))
        after = (restored.general_recovery, len(restored.recovery_events), restored.scenario)
        restored_strategy.reopened("BUY", D("4000.60"), "late-buy-s3", "late-buy-open")
        self.assertEqual((restored.general_recovery, len(restored.recovery_events),
                          restored.scenario), after)

    def test_double_sl_s1_transfers_pending_but_s2_d_is_not_duplicated(self):
        strategy, state = self.begin()
        strategy.stopped("BUY", D("3999.40"), "double-s1-buy")
        strategy.stopped("SELL", D("4001.10"), "double-s1-sell")
        self.assertEqual(strategy.account_double_sl_pending(), D("20"))
        self.assertEqual(strategy.account_double_sl_pending(), D("0"))
        strategy, state = self.begin()
        state.scenario = 2; state.general_recovery = D("20")
        strategy._targets_from_entries()
        strategy.stopped("BUY", self.sl_fill(state.long), "double-s2-buy")
        strategy.stopped("SELL", self.sl_fill(state.short), "double-s2-sell")
        before = state.general_recovery
        self.assertEqual(strategy.account_double_sl_pending(), D("0"))
        self.assertEqual(state.general_recovery, before)
        self.assertTrue(all(not item["reentry_accounted"] for item in state.pending_recovery))

    def test_continuation_pair_is_actual_once_target_unchanged_and_new_anchors(self):
        strategy, state = self.begin()
        state.scenario = 6; state.cycle_attempt = 2; state.general_recovery = D("176")
        target = state.target_value
        strategy.begin_continuation(D("4100.50"), D("4100.00"))
        self.assertEqual(state.general_recovery, D("176"))
        state.long.current_entry = D("4100.55")
        self.assertEqual(state.general_recovery, D("176"))
        strategy.confirm_continuation_fills(D("4100.50"), D("4100.00"))
        self.assertEqual((state.general_recovery, state.target_value), (D("186"), target))
        self.assertEqual((state.long.original_trigger_level, state.short.original_trigger_level),
                         (D("4100.50"), D("4100.00")))
        strategy.confirm_continuation_fills(D("4100.50"), D("4100.00"))
        self.assertEqual(state.general_recovery, D("186"))

    def test_reporting_distances_for_s1_s2_and_s9(self):
        leg = Leg("BUY", D("100"), D("100"), size=D("10"), stop_distance=D("4"))
        self.assertIn("recovery_distance=6", leg_details(
            leg, general_recovery=D("60"), scenario=1
        ))
        self.assertIn("recovery_distance=2", leg_details(
            leg, general_recovery=D("60"), scenario=5
        ))
        self.assertIn("ordinary Recovery TP отсутствует", leg_details(
            leg, general_recovery=D("60"), scenario=9
        ))

    def test_scenario_nine_actual_pnl_is_independent_from_general(self):
        for buy_close, expected in ((D("4000.00"), D("-387")),
                                    (D("4000.10"), D("-385"))):
            general = D("387")
            actual = D("-366")
            actual += (buy_close - D("4000.60")) * D("20")
            actual += (D("3999.90") - D("4000.35")) * D("20")
            self.assertEqual(general, D("387"))
            self.assertEqual(actual, expected)

    def test_inactive_v2_can_start_v3_but_active_v2_is_blocked(self):
        inactive = CycleState(recovery_model_version=2)
...
class BrokerEvidenceRegressionTest(unittest.TestCase):
    @staticmethod
    def recovery_bot(state_path: str):
        cfg = Settings(
            dry_run=False, api_key="key", identifier="id", password="password",
            state_file=state_path, scenario_sizes=(D("10"),) * 9,
            scenario_stop_distances=(D("1"),) * 9, target_profit=D("0.3"),
        )
        bot = Bot.__new__(Bot); bot.cfg = cfg; bot.capital = Mock(); bot.telegram = Mock()
        bot.state = CycleState(cycle_id=1, cycle_attempt=1, active_attempt_id=1)
        bot.strategy = Strategy(cfg, bot.state); bot.execution_policy = ExecutionPolicy()
        bot._missing_exit_since = None; bot.continuation = CycleContinuation(bot)
        bot.strategy.begin(D("4000.50"), D("4000.00"))
        bot.strategy.confirm_initial_fills(D("4000.50"), D("4000.00"))
        bot.state.long.deal_id, bot.state.short.deal_id = "buy-old", "sell-old"
        bot.state.remember_deal(bot.state.long, 1); bot.state.remember_deal(bot.state.short, 1)
        bot.strategy.stopped("SELL", D("4001.10"), "sell-s1")
        bot.strategy.reopened("SELL", D("3999.90"), "sell-new", "sell-reentry",
                              actual_size=D("10"))
        return bot

    @staticmethod
    def chronology_activity(open_time="2026-09-21T00:00:02.000Z",
                            close_time="2026-09-21T00:00:01.000Z"):
        return [
            {"dateUTC": open_time, "dealId": "sell-new", "source": "USER",
    ...
    def test_late_normal_tick_preserves_open_scenario_and_case_a_ledger(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(bot.state.general_recovery, D("20"))
            activity = self.chronology_activity()
            bot.capital.activity.return_value = activity
            bot.capital.positions.return_value = [{
                "position": {"dealId": "sell-new", "direction": "SELL", "level": 3999.90,
                             "size": 10, "stopLevel": 4000.90, "profitLevel": 3997.80},
                "market": {"epic": "GOLD"},
            }]
            bot._apply_protection = Mock(return_value=True)
            bot._create_trigger = Mock()
            ...
            self.assertEqual((closure["scenario_at_close"], closure["d_accounted"]), (1, False))
            self.assertEqual(bot.state.general_recovery, D("21"))
            before = (bot.state.general_recovery, dict(closure), bot.state.scenario)
            self.assertEqual(bot.strategy.projected_reopen("BUY"), (D("10"), D("1"), D("2.1")))
            self.assertEqual((bot.state.general_recovery, closure, bot.state.scenario), before)
            bot.state.save(bot.cfg.state_file)
            restored = CycleState.load(bot.cfg.state_file)
            Strategy(bot.cfg, restored).reopened(
                "BUY", D("4000.60"), "buy-new", "buy-reentry", actual_size=D("10")
            )
            self.assertEqual(restored.general_recovery, D("32"))
            Strategy(bot.cfg, restored).reopened(
                "BUY", D("4000.60"), "buy-new", "buy-reentry", actual_size=D("10")
    ...
    def test_late_stop_is_identical_in_startup_and_continuation_handlers(self):
        for owner in ("startup", "continuation"):
            with self.subTest(owner=owner), tempfile.TemporaryDirectory() as directory:
                ...
                if owner == "startup":
                    bot._migrate_recovery_model = Mock()
                    bot._recover_active_cycle({"sell-new": position}, [])
                else:
                    bot.state.continuation_managed = True
                    bot.state.continuation_stage = "ACTIVE"
                    bot.continuation.handle_active_scenario()
                closure = next(item for item in bot.state.pending_recovery
                               if item["deal_id"] == "buy-old")
                self.assertEqual((closure["scenario_at_close"], closure["d_accounted"],
                                  bot.state.general_recovery), (1, False, D("21")))

    def test_fast_path_prefers_activity_open_over_later_position_creation(self):
        with tempfile.TemporaryDirectory() as directory:
            bot = self.recovery_bot(str(Path(directory) / "state.json"))
            # Rewind to the one-sided state immediately before SELL reentry accounting.
            state = CycleState(cycle_id=1, cycle_attempt=1, active_attempt_id=1)
            ...
            stopped = bot.strategy.stopped("SELL", D("4001.10"), "sell-s1")
            stopped.trigger_id = "sell-trigger"
            state.pending_recovery[-1]["trigger_id"] = "sell-trigger"
            bot.capital.activity.return_value = self.chronology_activity(
                "2026-09-21T00:00:02.000Z", "2026-09-21T00:00:02.030Z"
            )
            bot._apply_protection = Mock(return_value=True); bot._create_trigger = Mock()
            candidate = {"dealId": "sell-new", "workingOrderId": "sell-trigger",
                         "direction": "SELL", "level": 3999.90, "size": 10,
                         "createdDateUTC": "2026-09-21T00:00:02.060Z"}
            self.assertTrue(bot._recover_trigger_fill_then_stop(
                {"sell-new": candidate}, state.long, stopped
            ))
    ...
    def test_automatic_trigger_relinks_closure_and_fill_once(self):
        with tempfile.TemporaryDirectory() as directory:
            bot = self.recovery_bot(str(Path(directory) / "state.json"))
            leg = bot.state.long
            # Use the unresolved BUY closure made by an explicit late S1 stop.
            ...
            self.assertEqual((bot.state.scenario, leg.deal_id, leg.current_entry, leg.size),
                             (before + 1, "buy-new", D("4000.60"), D("10")))
            general = bot.state.general_recovery
            bot._detect_trigger_fill({"buy-new": candidate})
            self.assertEqual((bot.state.scenario, bot.state.general_recovery),
                             (before + 1, general))

    def test_previous_v3_closures_without_owner_do_not_shadow_continuation(self):
        with tempfile.TemporaryDirectory() as directory:
            path = str(Path(directory) / "state.json")
            bot = self.recovery_bot(path)
            old = {"close_key": "old", "deal_id": "old-buy", "direction": "BUY",
                   "d_value": "10", "pending_d_value": "10", "d_accounted": True,
                   "reentry_accounted": False, "reopen_event_id": ""}
            bot.state.pending_recovery.append(old)
            bot.state.deal_history.append({"deal_id": "old-buy", "cycle_id": 1,
                                           "cycle_attempt": 1, "scenario": 1})
            bot.state.cycle_attempt = 2
            bot.state.long = Leg("BUY", D("4100.5"), D("4100.5"), deal_id="new-buy",
                                 size=D("10"), stop_distance=D("1"), stop=D("4099.5"))
            bot.state.short = Leg("SELL", D("4100"), D("4100"), deal_id="new-sell",
                                  size=D("10"), stop_distance=D("1"), stop=D("4101"))
            bot.state.remember_deal(bot.state.long, bot.state.scenario)
            bot.strategy.stopped("BUY", D("4099.4"), "new-stop")
            bot.state.save(path)
            restored = CycleState.load(path); strategy = Strategy(bot.cfg, restored)
            current = strategy._pending_for("BUY")
            self.assertEqual(current["deal_id"], "new-buy")
            self.assertTrue(old["d_accounted"])
    ...
    def test_401_retry_timeout_has_no_third_send_and_refresh_failure_stops(self):
        for second in (requests.ReadTimeout("retry unknown"),):
            client = CapitalClient(Settings(api_key="key", identifier="id", password="password"))
            client.last_login = 10**20
            client.http.request = Mock(side_effect=[self.response(401), second])
            client.http.post = Mock(return_value=self.response(200, headers={
                "CST": "new", "X-SECURITY-TOKEN": "new-sec",
            }))
            with self.assertRaises(CapitalError):
                client.request("DELETE", "/positions/deal")
            self.assertEqual(client.http.request.call_count, 2)
        failed = CapitalClient(Settings(api_key="key", identifier="id", password="password"))
        failed.last_login = 10**20
        failed.http.request = Mock(return_value=self.response(401))
        failed.http.post = Mock(return_value=self.response(401))
        with self.assertRaises(CapitalError):
            failed.request("GET", "/positions")
        self.assertEqual(failed.http.request.call_count, 1)
    ...
    def test_request_generation_is_bound_to_actual_token_headers(self):
        client = CapitalClient(Settings(api_key="key", identifier="id", password="password"))
        client.last_login = 10**20
        client.http.headers.update({"CST": "old-cst", "X-SECURITY-TOKEN": "old-sec"})
        sent = []
        def request(method, url, **kwargs):
            sent.append(dict(kwargs["headers"]))
            if len(sent) == 1:
                # Another worker refreshes after this request was prepared/sent.
                client.http.headers.update({"CST": "new-cst", "X-SECURITY-TOKEN": "new-sec"})
                client.session_generation = 1
                return self.response(401)
            return self.response(200)
        client.http.request = Mock(side_effect=request)
        client.http.post = Mock()
        self.assertEqual(client.request("DELETE", "/positions/deal"), {})
        self.assertEqual(sent, [
            {"CST": "old-cst", "X-SECURITY-TOKEN": "old-sec"},
            {"CST": "new-cst", "X-SECURITY-TOKEN": "new-sec"},
        ])
        client.http.post.assert_not_called()

    def test_two_full_concurrent_401_requests_share_one_refresh(self):
        threading = __import__("threading")
        client = CapitalClient(Settings(api_key="key", identifier="id", password="password"))
        client.last_login = 10**20
        client.http.headers.update({"CST": "old-cst", "X-SECURITY-TOKEN": "old-sec"})
        barrier = threading.Barrier(2); local = threading.local(); sent = []
        lock = threading.Lock()
        def request(method, url, **kwargs):
            count = getattr(local, "count", 0); local.count = count + 1
            with lock:
                sent.append((count, dict(kwargs["headers"])))
            if count == 0:
                barrier.wait()
                return self.response(401)
            return self.response(200)
        ...
        def run(method):
            try:
                client.request(method, "/positions", json={} if method != "GET" else None)
            except Exception as exc:
                errors.append(exc)
        threads = [threading.Thread(target=run, args=(method,))
                   for method in ("GET", "DELETE")]
        for thread in threads: thread.start()
        for thread in threads: thread.join(2)
        self.assertFalse(errors)
        self.assertEqual(client.http.post.call_count, 1)
        self.assertEqual(sum(1 for count, _ in sent if count == 0), 2)
        self.assertTrue(all(headers["CST"] == ("old-cst" if count == 0 else "new-cst")
                            for count, headers in sent))
    ...
    def test_transaction_snapshot_requires_every_expected_deal_and_finite_money(self):
        partial = broker_attempt_pnl([{
            "transactionId": "one", "dealId": "old-buy", "type": "TRADE",
            "amount": "-24", "currency": "USD",
        }], {"old-buy", "old-sell"})
        self.assertEqual((partial["status"], partial["amount"]), ("PARTIAL", None))
        complete = broker_attempt_pnl([
            {"transactionId": "one", "dealId": "old-buy", "type": "TRADE",
             "amount": "-24", "currency": "USD"},
            {"transactionId": "two", "dealId": "old-sell", "type": "TRADE",
             "amount": "-10", "currency": "USD"},
        ], {"old-buy", "old-sell"})
        self.assertEqual((complete["status"], complete["amount"]),
                         ("COMPLETE_SNAPSHOT", D("-34")))
        for invalid in ("invalid", "NaN", "Infinity", "-Infinity"):
            result = broker_attempt_pnl([{
                "transactionId": invalid, "dealId": "old-buy", "type": "TRADE",
                "amount": invalid, "currency": "USD",
            }], {"old-buy"})
            self.assertEqual((result["status"], result["amount"]), ("UNAVAILABLE", None))
        zero = broker_attempt_pnl([{
            "transactionId": "zero", "dealId": "old-buy", "type": "TRADE",
            "amount": "0", "currency": "USD",
        }], {"old-buy"})
        self.assertEqual((zero["status"], zero["amount"]),
                         ("COMPLETE_SNAPSHOT", D("0")))

    def test_transaction_worker_uses_owned_snapshot_and_all_documented_types(self):
        client = Mock()
        client.transactions.return_value = [
            {"transactionId": "a", "dealId": "attempt-one", "type": "TRADE",
             "amount": "-34", "currency": "USD"},
            {"transactionId": "foreign", "dealId": "attempt-two", "type": "TRADE",
             "amount": "3", "currency": "USD"},
        ]
        with patch("trader.notifications.time.time", return_value=200000):
            result = TransactionHistoryWorker._resolve(client, {
                "deal_ids": ["attempt-one"], "search_from_epoch": 100000,
            })
        self.assertEqual((result["status"], result["amount"]),
                         ("COMPLETE_SNAPSHOT", D("-34")))
        self.assertTrue(all(call.kwargs["all_types"] for call in client.transactions.call_args_list))
    ...
    def test_tick_never_performs_transaction_network_io(self):
        ...
        bot.state.pending_transaction_jobs = [{"key": "old", "deal_ids": ["old"]}]
        bot.tick()
        bot.capital.transactions.assert_not_called()

    def test_main_owner_applies_two_transaction_jobs_to_their_attempts_once(self):
        with tempfile.TemporaryDirectory() as directory:
            bot = Bot.__new__(Bot)
            bot.cfg = Settings(state_file=str(Path(directory) / "state.json"))
            bot.state = CycleState(
                attempt_history=[{"attempt_id": 1}, {"attempt_id": 2}],
                pending_transaction_jobs=[
                    {"key": "one", "attempt_id": 1, "deal_ids": ["old"],
                     "search_from_epoch": 1},
                    {"key": "two", "attempt_id": 2, "deal_ids": ["new"],
                     "search_from_epoch": 2},
                ],
            )
            bot.notification_worker = None
            ...
            bot.transaction_worker.results.return_value = [
                {"key": "one", "result": {"status": "COMPLETE_SNAPSHOT",
                 "amount": D("-34"), "currency": "USD", "components": [],
                 "observed_to_epoch": 3}},
                {"key": "two", "result": {"status": "COMPLETE_SNAPSHOT",
                 "amount": D("3"), "currency": "USD", "components": [],
                 "observed_to_epoch": 4}},
            ]
            bot.telegram = Mock(); bot._queue_pending_reports = Mock()
            bot._tick_notifications()
            self.assertEqual(
            ...
            bot._tick_notifications()
            self.assertEqual(json.dumps(bot.state.attempt_history, sort_keys=True), snapshot)


class Attempt285ProductionRegressionTest(unittest.TestCase):
    def make_bot(self, path, stopped_direction="BUY"):
        cfg = Settings(
            dry_run=False, api_key="key", identifier="id", password="password",
            state_file=str(path), scenario_sizes=(D("10"),) * 9,
            scenario_stop_distances=(D("1"), D("2"), *(D("3"),) * 7),
            target_profit=D("0.30"),
        )
        bot = Bot.__new__(Bot); bot.cfg = cfg; bot.capital = Mock(); bot.telegram = Mock()
        bot.state = CycleState(cycle_id=285, cycle_attempt=1, active_attempt_id=285)
        bot.strategy = Strategy(cfg, bot.state); bot.continuation = CycleContinuation(bot)
        bot.execution_policy = ExecutionPolicy(); bot._missing_exit_since = None
        bot.strategy.begin(D("4279.94"), D("4279.37"))
        bot.strategy.confirm_initial_fills(D("4279.94"), D("4279.37"))
        bot.state.long.deal_id, bot.state.short.deal_id = "buy-old", "sell-old"
        bot.state.remember_deal(bot.state.long, 1); bot.state.remember_deal(bot.state.short, 1)
        closed = bot.state.long if stopped_direction == "BUY" else bot.state.short
        fill = D("4278.81") if stopped_direction == "BUY" else D("4280.50")
        bot.strategy.stopped(stopped_direction, fill, f"{stopped_direction}-s1")
        closed.trigger_id = f"{stopped_direction.lower()}-order"
        bot.state.cycle_trigger_ids.append(closed.trigger_id)
        bot.state.pending_recovery[-1]["trigger_id"] = closed.trigger_id
        return bot

    @staticmethod
    def wrapper(data):
        return {"position": data, "market": {"epic": "GOLD"}}

    def test_normal_dispatcher_reentry_then_late_sl_uses_working_order_clock_after_restart(self):
        for restart in (False, True):
            with self.subTest(restart=restart), tempfile.TemporaryDirectory() as directory:
                path = Path(directory) / "state.json"
                bot = self.make_bot(path)
                execution = {
                    "dateUTC": "2026-09-23T17:33:57.928Z", "dealId": "buy-order",
                    "source": "USER", "type": "WORKING_ORDER", "status": "EXECUTED",
                    "details": {"direction": "BUY", "size": 10, "level": 4279.94},
                }
                buy = {"dealId": "buy-new", "workingOrderId": "buy-order",
                       "direction": "BUY", "level": 4280.07, "size": 10,
                       "stopLevel": 4278.07, "profitLevel": 4282.20,
                       "createdDateUTC": "2026-09-23T17:33:58.012Z"}
                sell = {"dealId": "sell-old", "direction": "SELL", "level": 4279.37,
                        "size": 10, "stopLevel": 4281.37, "profitLevel": 4277.24}
                bot.capital.activity.return_value = [execution]
                bot._apply_protection = Mock(return_value=True)
                bot._detect_trigger_fill({"buy-new": buy, "sell-old": sell})
                self.assertEqual((bot.state.scenario, bot.state.general_recovery), (2, D("21.30")))
                opened = next(item for item in bot.state.deal_history
                              if item.get("deal_id") == "buy-new")
                self.assertEqual((opened["trigger_id"], opened["broker_open_execution_time"]),
                                 ("buy-order", "2026-09-23T17:33:57.928000+00:00"))
                bot.state.save(str(path))
                if restart:
                    state = CycleState.load(str(path))
                    bot.state = state; bot.strategy = Strategy(bot.cfg, state)
                    bot.continuation = CycleContinuation(bot)
                sell_sl = {
                    "dateUTC": "2026-09-23T17:34:37.146Z", "dealId": "sell-old",
                    "source": "SL", "type": "POSITION", "status": "ACCEPTED",
                    "details": {"direction": "BUY", "level": 4281.43, "size": 10,
                                "stopLevel": 4281.37, "profitLevel": 4277.24},
                }
                reads = {"sell-old": 0}
                def activity(deal_id="", **_kwargs):
                    if deal_id == "sell-old":
                        reads["sell-old"] += 1
                        return [] if reads["sell-old"] == 1 else [sell_sl]
                    return [execution, sell_sl]
                bot.capital.activity.side_effect = activity
                bot.capital.positions.return_value = [self.wrapper(buy)]
                bot.capital.working_stop.return_value = "sell-trigger-ref"
                bot.capital.working_orders.return_value = []
                bot.capital.wait_confirmation.return_value = {
                    "dealStatus": "ACCEPTED", "dealId": "sell-trigger"
                }
                bot._apply_protection = Mock(return_value=True)
                with patch("trader.app.time.sleep"):
                    bot._tick_cycle()
                self.assertFalse(bot.state.manual)
                self.assertEqual((bot.state.scenario, bot.state.general_recovery), (2, D("41.90")))
                self.assertEqual((bot.state.long.deal_id, bot.state.long.current_entry,
                                  bot.state.long.size, bot.state.long.take_profit),
                                 ("buy-new", D("4280.07"), D("10"), D("4284.26")))
                self.assertEqual((bot.state.short.trigger_id,
                                  bot.state.short.original_trigger_level),
                                 ("sell-trigger", D("4279.37")))
                mutations = bot.capital.working_stop.call_count
                bot._tick_cycle()
                self.assertEqual((bot.state.general_recovery, bot.state.scenario,
                                  bot.capital.working_stop.call_count),
                                 (D("41.90"), 2, mutations))

    def test_mirror_late_sl_after_reentry_is_accounted_once(self):
        with tempfile.TemporaryDirectory() as directory:
            bot = self.make_bot(Path(directory) / "s.json", "SELL")
            execution = {"dateUTC": "2026-09-23T10:00:00Z", "dealId": "sell-order",
                         "source": "USER", "type": "WORKING_ORDER", "status": "EXECUTED"}
            sell_new = {"dealId": "sell-new", "workingOrderId": "sell-order",
                        "direction": "SELL", "level": 4279.20, "size": 10}
            bot.capital.activity.return_value = [execution]
            bot._apply_protection = Mock(return_value=True)
            bot._detect_trigger_fill({"sell-new": sell_new})
            buy_sl = {"dateUTC": "2026-09-23T10:00:10Z", "dealId": "buy-old",
                      "source": "SL", "type": "POSITION", "status": "ACCEPTED",
                      "details": {"direction": "SELL", "level": 4277.80, "size": 10,
                                  "stopLevel": 4277.94}}
            bot.capital.activity.return_value = [execution, buy_sl]
            event = find_close_event([buy_sl], "buy-old", "SL")
            before = bot.state.general_recovery
            self.assertIsNotNone(bot._apply_confirmed_stop_event(bot.state.long, event,
                                                                 [execution, buy_sl]))
            after = bot.state.general_recovery
            self.assertGreater(after, before)
            stop_key = next(key for key in bot.state.processed_events
                            if key.startswith("stop:buy-old:"))
            self.assertIs(bot.strategy.stopped("BUY", D("4277.80"), stop_key), bot.state.long)
            self.assertEqual(bot.state.general_recovery, after)

    def test_confirmed_close_evidence_survives_empty_history_and_records_range(self):
        with tempfile.TemporaryDirectory() as directory:
            bot = self.make_bot(Path(directory) / "s.json")
            event_payload = {
                "dateUTC": "2026-09-23T17:34:37.146Z", "dealId": "sell-old",
                "source": "SL", "type": "POSITION", "status": "ACCEPTED",
                "details": {"level": 4281.43, "size": 10, "stopLevel": 4281.37},
            }
            bot.capital.activity.side_effect = [[event_payload], []]
            self.assertEqual(bot._closing_fill(bot.state.short, "SL"), D("4281.43"))
            self.assertEqual(bot._closing_fill(bot.state.short, "SL"), D("4281.43"))
            record = next(item for item in bot.state.deal_history
                          if item.get("deal_id") == "sell-old")
            self.assertEqual(record["protection_range"]["range_assessment"], "SL")
            self.assertEqual(record["close_execution_time"],
                             "2026-09-23T17:34:37.146000+00:00")

    def test_protection_range_is_diagnostic_only(self):
        base = {"dateUTC": "2026-01-01T00:00:00Z", "dealId": "owned",
                "source": "USER", "type": "POSITION", "status": "ACCEPTED",
                "details": {"direction": "SELL", "level": 100.5}}
        event = normalize_event(base)
        result = protection_range_diagnostic(
            event, confirmed_stop=D("99.50"), confirmed_take_profit=D("100.00")
        )
        self.assertEqual((result["range_assessment"], result["authoritative_source"]),
                         ("TP", "USER"))
        for direction, source, fill, expected in (
            ("BUY", "SL", "99.50", "SL"), ("SELL", "SL", "100.50", "SL"),
            ("BUY", "TP", "101.50", "TP"), ("SELL", "TP", "98.50", "TP"),
        ):
            payload = {**base, "source": source,
                       "details": {"direction": direction, "level": fill}}
            diagnostic = protection_range_diagnostic(
                normalize_event(payload),
                confirmed_stop=D(fill) + (D("0.50") if source == "SL" else D("10")),
                confirmed_take_profit=D(fill) - (D("0.50") if source == "TP" else D("10")),
            )
            self.assertEqual(diagnostic["range_assessment"], expected)
        outside = protection_range_diagnostic(
            normalize_event({**base, "source": "SL", "details": {"level": 102}}),
            confirmed_stop=D("100"), confirmed_take_profit=None,
        )
        self.assertEqual(outside["range_assessment"], "NONE")


class DurableMutationAndTransactionRegressionTest(unittest.TestCase):
    def one_sided_bot(self, path):
        cfg = Settings(dry_run=False, api_key="k", identifier="i", password="p",
                       state_file=str(path), scenario_sizes=(D("10"),) * 9,
                       scenario_stop_distances=(D("1"),) * 9)
        bot = Bot.__new__(Bot); bot.cfg = cfg; bot.telegram = Mock(); bot.capital = Mock()
        bot.state = CycleState(active=True, scenario=1, phase="LONG_ONLY", cycle_id=1,
                               cycle_attempt=1, active_attempt_id=1)
        bot.state.long = Leg("BUY", D("100"), D("100"), deal_id="survivor", size=D("10"),
                             stop_distance=D("1"), stop=D("99"), take_profit=D("102"))
        bot.state.short = Leg("SELL", D("99.5"), D("99.5"), deal_id="closed", open=False,
                              size=D("10"), stop_distance=D("1"), trigger_id="old-order",
                              trigger_reference="old-ref")
        bot.state.pending_recovery = [{"close_key": "close", "deal_id": "closed",
            "direction": "SELL", "d_value": "10", "pending_d_value": "10",
            "d_accounted": False, "reentry_accounted": False, "trigger_id": "old-order",
            "cycle_id": 1, "cycle_attempt": 1, "original_trigger_anchor": "99.5"}]
        bot.state.cycle_trigger_ids = ["old-order"]
        bot.strategy = Strategy(cfg, bot.state); bot.continuation = CycleContinuation(bot)
        bot.execution_policy = ExecutionPolicy(); bot._missing_exit_since = None
        return bot
    ...
    def test_cancel_unknown_keeps_binding_then_suppresses_automatic_recreation(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            bot.capital.delete_working_order.return_value = False
            with self.assertRaisesRegex(RuntimeError, "ещё не подтверждён"):
                bot._manual_command("/canceltrigger", ["short"])
            self.assertEqual(bot.state.short.trigger_id, "old-order")
    ...
    def test_unknown_trigger_cancellation_does_not_stop_survivor_protection(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            bot.capital.positions.return_value = [{"position": survivor,
                                                   "market": {"epic": "GOLD"}}]
            bot.capital.activity.return_value = []
            bot._protection_matches = Mock(return_value=False)
            bot._apply_protection = Mock(return_value=True)
            bot._tick_cycle()
            bot._apply_protection.assert_called_with(bot.state.long)
            self.assertEqual(bot.state.short.trigger_id, "old-order")
            bot.capital.working_stop.assert_not_called()

    def test_trigger_tp_race_uses_actual_size_in_real_caller(self):
        with tempfile.TemporaryDirectory() as directory:
            bot = self.one_sided_bot(Path(directory) / "state.json")
            bot.state.short.size = D("10")
            bot.capital.delete_working_order.return_value = False
            opened = {"dateUTC": "2026-01-01T00:00:01Z", "dealId": "race",
                      "source": "USER", "type": "POSITION", "status": "ACCEPTED",
                      "details": {"workingOrderId": "old-order", "direction": "SELL",
                                  "level": 100, "size": 7}}
            bot.capital.activity.return_value = [opened]
            bot.capital.positions.return_value = [{"position": {"dealId": "race",
                "workingOrderId": "old-order", "direction": "SELL", "level": 100,
                "size": 7}, "market": {"epic": "GOLD"}}]
            bot.capital.close_position.return_value = "close-ref"
            bot.capital.wait_confirmation.return_value = {
                "dealStatus": "ACCEPTED", "dealId": "race", "level": 101,
            }
    ...
    def test_real_transaction_payload_and_incomplete_fee_only(self):
        payload = {"reference": "139712570466515", "transactionType": "TRADE",
                   "size": "-11.2", "currency": "USD", "dealId": "buy-284",
                   "status": "PROCESSED", "dateUtc": "2026-09-23T17:25:00.744"}
        result = broker_attempt_pnl([payload], {"buy-284"})
        self.assertEqual((result["status"], result["amount"], result["currency"]),
                         ("COMPLETE_SNAPSHOT", D("-11.2"), "USD"))
        fee_only = broker_attempt_pnl([{
            "transactionId": "fee", "dealId": "buy-284", "type": "SWAP",
            "amount": "-1", "currency": "USD", "status": "PROCESSED",
        }], {"buy-284"})
        self.assertEqual((fee_only["status"], fee_only["amount"]), ("PARTIAL", None))
        shared = broker_attempt_pnl([payload, {**payload, "dealId": "sell-285",
            "size": "-20.6"}], {"buy-284", "sell-285"})
        self.assertEqual(shared["status"], "COMPLETE_SNAPSHOT")
        self.assertEqual(len(shared["components"]), 2)

    def test_real_transaction_payload_flows_from_worker_to_main_owner(self):
        with tempfile.TemporaryDirectory() as directory:
            payload = {"reference": "139712570466515", "transactionType": "TRADE",
                       "size": "-11.2", "currency": "USD", "dealId": "buy-284",
                       "status": "PROCESSED", "dateUtc": "2026-09-23T17:25:00.744"}
            client = Mock(); client.transactions.return_value = [payload]
            with patch("trader.notifications.time.time", return_value=200):
                result = TransactionHistoryWorker._resolve(client, {
                    "deal_ids": ["buy-284"], "search_from_epoch": 100,
                })
            bot = Bot.__new__(Bot); bot.cfg = Settings(
                state_file=str(Path(directory) / "state.json")
            )
            bot.state = CycleState(attempt_history=[{"attempt_id": 284}],
                pending_transaction_jobs=[{"key": "tx", "attempt_id": 284,
                    "deal_ids": ["buy-284"], "search_from_epoch": 100,
                    "next_check_at": 0}])
            bot.notification_worker = None; bot.telegram = Mock()
            bot._queue_pending_reports = Mock(); bot.transaction_worker = Mock()
            bot.transaction_worker.results.return_value = [{"key": "tx", "result": result}]
            bot._tick_notifications(now=201)
            attempt = bot.state.attempt_history[0]
            self.assertEqual((attempt["broker_transaction_status"],
                              attempt["broker_transaction_pnl"],
                              attempt["broker_transaction_currency"]),
                             ("COMPLETE_SNAPSHOT", "-11.2", "USD"))

    def test_identical_snapshot_advances_watermark_without_reaccounting(self):
        with tempfile.TemporaryDirectory() as directory:
            bot = Bot.__new__(Bot); bot.cfg = Settings(state_file=str(Path(directory) / "s.json"))
            result = {"status": "COMPLETE_SNAPSHOT", "amount": D("-11.2"),
                      "currency": "USD", "components": [], "observed_to_epoch": 20}
            fingerprint = transaction_result_fingerprint(result)
            bot.state = CycleState(attempt_history=[{"attempt_id": 1,
                "broker_transaction_pnl": "-11.2"}], pending_transaction_jobs=[{
                "key": "one", "attempt_id": 1, "deal_ids": ["d"],
                "search_from_epoch": 1, "observed_to_epoch": 10,
                "result_fingerprint": fingerprint, "next_check_at": 0,
            }])
            bot.notification_worker = None; bot.telegram = Mock()
            bot._queue_pending_reports = Mock(); bot.transaction_worker = Mock()
            bot.transaction_worker.results.return_value = [{"key": "one", "result": result}]
            before = json.dumps(bot.state.attempt_history, sort_keys=True)
            bot._tick_notifications(now=20)
            self.assertEqual(bot.state.pending_transaction_jobs[0]["observed_to_epoch"], 20)
            self.assertEqual(json.dumps(bot.state.attempt_history, sort_keys=True), before)

    def test_worker_key_remains_busy_until_main_acknowledges(self):
        worker = TransactionHistoryWorker(Settings(), client_factory=lambda _cfg: Mock())
        worker._resolve = Mock(return_value={"status": "PENDING", "amount": None,
                                             "currency": "", "components": [],
                                             "observed_to_epoch": 1})
        worker.start()
        try:
            job = {"key": "race", "deal_ids": ["d"], "search_from_epoch": 0}
            worker.submit(job)
            ...
            worker.acknowledge("race")
            worker.submit(job)
            deadline = time.time() + 2
            while time.time() < deadline and not worker.results():
                time.sleep(0.001)
